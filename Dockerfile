...
RUN apk update && apk add --no-cache bash curl wget tar ca-certificates shadow gzip && apk upgrade ca-certificates
...
RUN curl -L -o dockle.tar.gz https://github.com/goodwithtech/dockle/releases/download/v0.4.15/dockle_0.4.15_Linux-64bit.tar.gz 
RUN tar -zxf dockle.tar.gz && mv dockle /usr/local/bin/dockle && chmod +x /usr/local/bin/dockle && rm -rf dockle.tar.gz

# install kubesec
RUN curl -L -o kubesec.tar.gz https://github.com/controlplaneio/kubesec/releases/download/v2.14.2/kubesec_linux_amd64.tar.gz
